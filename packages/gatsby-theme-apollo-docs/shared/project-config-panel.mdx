...
To get an API key, you will need to [log in to Graph Manager](https://engine.apollographql.com) and create a new graph by clicking the "New Graph" button. If you already have a graph, get your API key by visiting your graph's settings page. Once you have your API key, add it to your `.env` file like so:

```
ENGINE_API_KEY=service:foobar:d1rzyrmanmrZXxTTQLxghX
```

The Apollo CLI will be looking for your `.env` file because it uses your Graph Manager API key to authenticate with the schema registry when it pushes your schema.
...
The [Apollo config documentation](https://www.apollographql.com/docs/references/apollo-config/#clientservice) has more details and advanced configuration options for the `apollo.config.js` format.
